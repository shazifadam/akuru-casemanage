  ...
  let query = supabase
    ...
    .select(`
      *,
      ...
      font:fonts(id, name, contributor_id,
        contributor:contributors(id, name)
      )
    `)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {licenses?.length ?? 0} license{licenses?.length !== 1 ? "s" : ""} · Total: {mvr(totalRevenue)}
          </p>
        </div>
      ...
      <form className="flex flex-wrap items-center gap-2">
        ...
        />
        <select name="font" defaultValue={params.font ?? ""} className="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
          <option value="">All fonts</option>
          {(fonts ?? []).map((f) => (
            <option key={f.id} value={f.id}>{f.name}</option>
          ))}
        </select>
        <select name="status" defaultValue={params.status ?? ""} className="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
          <option value="">All statuses</option>
          <option value="pending">Pending</option>
          <option value="paid">Paid</option>
          <option value="overdue">Overdue</option>
        </select>
        <select name="source" defaultValue={params.source ?? ""} className="h-8 rounded-md border border-input bg-background px-2 text-xs focus:outline-none focus:ring-1 focus:ring-ring">
          <option value="">All sources</option>
          <option value="direct_sale">Direct Sale</option>
          <option value="enforcement">Enforcement</option>
          <option value="election_case">Election Case</option>
        </select>
        <button type="submit" className="h-8 rounded-md border border-input bg-background px-3 text-xs hover:bg-accent transition-colors">
          Filter
        </button>
        {(params.font || params.status || params.source || params.q) && (
          <Link href="/licenses" className="h-8 flex items-center px-3 text-xs text-muted-foreground hover:text-foreground">
            Clear
          </Link>
        )}
      </form>
              ...
              {(licenses ?? []).map((l) => (
                <tr key={l.id} className="group hover:bg-muted/20 transition-colors">
                  <td className="px-4 py-3">
                    <Link href={`/licenses/${l.id}`} className="font-mono text-xs font-medium text-primary hover:underline">
                      {l.license_number}
                    </Link>
                    {l.is_fine && (
                      <span className="ml-1.5 rounded bg-red-100 px-1 py-0.5 text-[10px] font-medium text-red-700">FINE</span>
                    )}
                  </td>
                  <td className="px-4 py-3">
                    <div className="text-xs font-medium">{(l.buyer as any)?.name ?? "—"}</div>
                    {(l.buyer as any)?.organization && (
                      <div className="text-[10px] text-muted-foreground">{(l.buyer as any).organization}</div>
                    )}
                  </td>
                  <td className="px-4 py-3 text-xs text-muted-foreground">{(l.font as any)?.name ?? "—"}</td>
                  ...
                  <td className="px-4 py-3"><LicensePaymentBadge status={l.payment_status as PaymentStatus} /></td>
                  <td className="px-4 py-3"><LicenseSourceBadge source={l.source as LicenseSource} /></td>
                  <td className="px-4 py-3">
